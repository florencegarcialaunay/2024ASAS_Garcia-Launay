...
class Batch:
    ...
    def delivery(self, Feeds, param, input, function_LCA_pig, CF, background, file_perf, file_lca, i):
        ...
        for name in list_delivery:
            self.dict_pigs[name] = function_LCA_pig(self.dict_pigs[name], Feeds, param, input, CF, background)
        ...
        for name in list_delivery:
            ...
            print(param['scenario'] + ' ' + p.ID + ' ' + p.name + ' ' + '%.*f' % (3, p.CC_feed) + ' ' + '%.*f' % (3, p.AC_feed)
                  + ' ' + '%.*f' % (3, p.CC_gain) + ' ' + '%.*f' % (3, p.AC_gain) , end=' ', file=file_lca)
            print(' ', file=file_lca)
            file_lca.flush()
...
class Pig:
    ...
    def __init__(self, profile, Ageimoy, BWimoy, dict_INRAPorc, ID=' '):
        ...
        # intializing variables for calculating emissions and LCA impacts
        self.N_NH3EmiH = 0.0 # emissions N-NH3 in the building - kg
        self.N_N2OEmiH = 0.0 # emissions N-N2O in the building - kg
        self.N_NOxEmiH = 0.0  # emissions N-NOx in the building - kg
        self.N_N2H = 0.0 # emissions N-N2 in the building - kg
        self.N_St = 0.0  ### The nitrogen remaining in manure at the starting of manure storage - kg
        self.TAN_St = 0.0  ### The total ammoniacal nitrogen remaining in manure at the starting of manure storage - kg
        self.N_NH3EmiSt = 0.0 # emissions N-NH3 at manure storage - kg
        self.N_N2OEmiSt = 0.0  # emissions N-N2O at manure storage - kg
        self.N_NOxEmiSt = 0.0  # emissions N-NOx at manure storage - kg

        self.CH4_enteric = 0.0  # The methane CH4 emissions from enteric fermentation
        self.CH4_St = 0.0  # The methane CH4 emissions from manure.
        
        self.CC_feed = 0.0 # climate change impact of feed consumed - kg CO2-eq/pig - according to EF 3.0
        self.AC_feed = 0.0 # acidification impact of feed consumed - kg CO2-eq/pig - according to EF 3.0
        self.CC_gain = 0.0 # climate change impact - kg CO2-eq/kg gain - according to EF 3.0
        self.AC_gain = 0.0 # acidification impact - mol H+-eq/kg gain - according to EF 3.0
...
class Feed:
    ...
    def __init__(self, dict_Feed):
        ...
        self.CC = dict_Feed['CC'] # climate change impact of the feed in kgCO2-eq / kg feed
        self.AC = dict_Feed['AC'] # acidification impact of the feed in molH+ / kg feed 
